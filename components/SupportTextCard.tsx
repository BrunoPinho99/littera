...
const SupportTextCard: React.FC<SupportTextCardProps> = ({ data, compact = false }) => {
  ...
  return (
    <>
      <div className={`group relative bg-[#F8F9FE] dark:bg-white/5 ghost-border rounded-card ${compact ? 'p-5' : 'p-8'} text-left transition-all duration-300 hover:shadow-lg hover:border-primary/20 flex flex-col`}>
        <div className="relative z-10">
          <div className={`flex items-center gap-3 ${compact ? 'mb-2' : 'mb-4'}`}>
            <div className={`${compact ? 'w-8 h-8 rounded-lg' : 'w-10 h-10 rounded-xl'} bg-surface-container-lowest text-primary flex items-center justify-center shadow-sm border border-gray-50 dark:border-white/5 group-hover:bg-primary group-hover:text-white transition-all`}>
              <span className={`material-icons-outlined ${compact ? 'text-sm' : 'text-[22px]'}`}>
                {data.icon === 'article' ? 'description' : data.icon}
              </span>
            </div>
            <h4 className={`font-bold ${compact ? 'text-sm' : 'text-[17px]'} text-on-surface group-hover:text-primary transition-colors`}>
              {data.title}
            </h4>
          </div>
          <p className={`${compact ? 'text-[12px]' : 'text-[14.5px]'} text-on-surface-variant dark:text-on-surface-variant leading-relaxed font-medium`}>
            {previewText}
          </p>
        </div>
          ...
          <div className="relative bg-surface-container-lowest w-full max-w-2xl rounded-card shadow-2xl overflow-hidden animate-scale-up">
            <div className="bg-primary/5 p-8 border-b border-gray-50 dark:border-slate-800 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-black text-on-surface text-xl tracking-tight">{data.title}</h3>
                  <p className="text-[10px] font-black text-primary uppercase tracking-widest">Fonte Motivadora</p>
                </div>
              </div>
              <button 
                onClick={() => setIsModalOpen(false)}
                className="w-10 h-10 rounded-full bg-surface-container-lowest text-on-surface-variant hover:text-rose-500 hover:bg-rose-50 transition-all flex items-center justify-center shadow-sm"
              >
