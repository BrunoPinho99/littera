...
const CheckoutWizard: React.FC<{ onBack: () => void; onLogin: () => void }> = ({ onBack, onLogin }) => {
  ...
  const handleNextStep = async () => {
    setGlobalError(null);
    if (step === 1) {
      const isValid = await trigger(['directorName', 'schoolName', 'email', 'phone', 'password', 'confirmPassword', 'cpfCnpj']);
                ...
                {step === 1 && (() => {
                  ...
                  return (
                    <div className="space-y-4 animate-fade-in">
                      ...
                      {renderField('CPF ou CNPJ', 'cpfCnpj', 'text', '000.000.000-00', formatCpfCnpj)}
                    </div>
                  );
                })()}
