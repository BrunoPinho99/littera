  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-4">
        ...
        <p className="text-on-surface-variant font-bold animate-pulse uppercase tracking-widest text-xs">Calculando Posições...</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in max-w-5xl mx-auto pb-20 px-4">
      ...
      <div className="text-center mb-12">
        ...
        {!manualData && (
          <div className="bg-surface-container-high p-1.5 rounded-2xl inline-flex gap-1 shadow-inner mt-6">
      ...
      <div className="bg-surface-container-lowest rounded-card shadow-ambient ghost-border overflow-hidden">
        ...
        <div className="">
          ...
          {visibleList.length === 0 && (
            <div className="py-20 text-center text-on-surface-variant font-bold">Nenhum dado disponível.</div>
          )}
        </div>

        {/* Pagination Footer */}
        {totalPages > 1 && (
          <div className="px-8 py-6 bg-surface-container-low flex justify-center items-center gap-4">
            <button 
              onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
              disabled={currentPage === 1}
              className="p-2 rounded-xl hover:bg-white hover:shadow-sm disabled:opacity-30 transition-all text-on-surface-variant"
            >
